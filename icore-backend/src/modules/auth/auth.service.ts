...
export class AuthService {
    ...
    async login(user: User, response: Response) {
        ...
        const expiresAccessToken = new Date(Date.now() + accessTokenExpiresInMins * 60 * 1000);
        const expiresRefreshToken = new Date(Date.now() + refreshTokenExpiresInMins * 60 * 1000);
